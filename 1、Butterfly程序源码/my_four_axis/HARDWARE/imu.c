...
void mahony_update(float gx, float gy, float gz, float ax, float ay, float az) 
{
    ...
    //z轴角速度积分的偏航角
    att.yaw += _Mpu.deg_s.z  * att_time.delta_time_ms*0.001f;   
...
//旋转矩阵：机体坐标系 -> 地理坐标系
static void rotation_matrix(void)
{
...
static void rotation_matrix_T(void)
{
...
void Matrix_ready(void)
{
	...
	get_nav_acc(_acc_fix_lpf);//导航系下三轴加速度  cm/s^2
 // sins_high();//竖直方向惯导融合
}
