...
static void USART2_Init(u32 bound)
{
	...
	/*      设置中断优先级       */
	NVIC_InitStructure.NVIC_IRQChannel=USART2_IRQn;//选择通道2
...
void uart2_init(u32 bound)
{
	...
	DMA_Init(DMA1_Channel6,&DMA_InitStructure);//初始化 DMA1 通道6 
	
	
	/*USART1_TX-发送-DMA1 通道7*/ 
	...
	DMA_InitStructure.DMA_PeripheralDataSize	=DMA_PeripheralDataSize_Byte;//外设数据宽度		外设数据长度1byte
	DMA_InitStructure.DMA_PeripheralInc			=DMA_PeripheralInc_Disable;	//外设增量模式	   	外设地址固定	
	DMA_Init(DMA1_Channel7,&DMA_InitStructure);//初始化 DMA1 通道7 
	
	/*DMA 中断优先级配置*/
	NVIC_InitStructure.NVIC_IRQChannel                   = DMA1_Channel6_IRQn;//USART1_RX
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 3;                //抢占优先级
	NVIC_InitStructure.NVIC_IRQChannelSubPriority        = 1;                //子优先级
	NVIC_InitStructure.NVIC_IRQChannelCmd                = ENABLE;
	NVIC_Init(&NVIC_InitStructure);

	NVIC_InitStructure.NVIC_IRQChannel                   = DMA1_Channel7_IRQn;//USART1_TX
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 3;                //抢占优先级
	NVIC_InitStructure.NVIC_IRQChannelSubPriority        = 2;                //子优先级
	NVIC_Init(&NVIC_InitStructure);
	DMA_ITConfig(DMA1_Channel6,DMA_IT_TC,ENABLE);//开启中断
	DMA_ITConfig(DMA1_Channel7,DMA_IT_TC,ENABLE);//开启中断
	
	
	/*DMA使能*/
	DMA_Cmd(DMA1_Channel6,ENABLE);//DMA使能 DMA1 通道6 
	DMA_Cmd(DMA1_Channel7,ENABLE);//DMA使能 DMA1 通道7
	
	USART_DMACmd(USART2,USART_DMAReq_Rx,ENABLE);//串口2 DMA初始化
	USART_DMACmd(USART2,USART_DMAReq_Tx,ENABLE);//串口2 DMA初始化
...
void DMA_SendData(uint8_t *data,uint16_t len)
{
	...
	DMA_SendOnce(DMA1_Channel7,len);
}
...
void DMA1_Channel6_IRQHandler(void)
{
	...
	{
//		LED0=0;
		DMA_Cmd(DMA1_Channel6, DISABLE);//关闭 DMA1 通道1 传输
		
		DMA_SetCurrDataCounter(DMA1_Channel6, 32);
...
void USART2_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART2,USART_IT_IDLE) != RESET)
	{
		...
		temp = USART2->SR;
		temp = USART2->DR;  //空闲中断的清除比较特别，不是写寄存器，而是读寄存器

		USART_RX_Len = 32 - DMA_GetCurrDataCounter(DMA1_Channel6);//获取收到的数据长度
		
		DMA_SetCurrDataCounter(DMA1_Channel6, 32);//重新设置数据传输量
...
{  
	...
	{
		USART_TX_FLAG = 0;
		DMA_ClearITPendingBit(DMA1_IT_GL7);//清除通道7传输完成标志
	}
}
