...
static void DataConversion(unsigned short int* adc_dat)
{
	//油门数据范围转换为0~1000
	//打舵数据范围转换为0~100
    _RC.thr = (uint16_t)(1000.0f - ((float)(adc_dat[3]))*0.24420f);
    
    _RC.pit = (uint8_t)(((float)(adc_dat[0]))*0.02442f);
    _RC.rol = (uint8_t)(((float)(adc_dat[1]))*0.02442f);
	
		_RC.yaw = (uint8_t)(((float)(adc_dat[2]))*0.02442f);
}
